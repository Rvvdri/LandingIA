...
module.exports = async function handler(req, res) {
  ...
  // Productos con sus imágenes embebidas directamente en el prompt como src base64
  let productosPrompt = '';
  if (productos?.length > 0) {
    productosPrompt = '\nPRODUCTOS/SERVICIOS (usa las imágenes base64 como src en los img tags de cada tarjeta):\n';
    productosPrompt += productos.map((p, i) => {
      let linea = `${i+1}. Nombre: ${p.nombre} | Precio: ${p.precio} | Descripción: ${p.descripcion}`;
      if (p.imagenes?.length > 0) {
        linea += `\n   Imágenes (${p.imagenes.length}): usa estos src exactos en orden:\n`;
        linea += p.imagenes.map((img, j) => `   img${j+1}: ${img.base64}`).join('\n');
      }
      return linea;
    }).join('\n\n');
  }

  // Imágenes generales (máx 3 para no exceder el límite)
  let imagenesGeneralesPrompt = '';
  if (imagenes?.length > 0) {
    const maxImgs = imagenes.slice(0, 3);
    imagenesGeneralesPrompt = '\nFOTOS DEL NEGOCIO (usa estos src en la galería o hero):\n';
    imagenesGeneralesPrompt += maxImgs.map((img, i) => `foto${i+1}: ${img.base64}`).join('\n');
  }

  const promptTexto = `Eres un experto diseñador web. Crea una página web HTML completa, moderna y profesional.
...
- Redes: ${redesSociales || 'No indicadas'}
...
- Usa las imágenes base64 proporcionadas directamente como src
- Si no hay imagen para un producto, usa un SVG placeholder de color
